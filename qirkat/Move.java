...
class Move {
     ...
     */
    static Move move(char col0, char row0, char col1, char row1, Move nextJump)
    {
        if (_staged == null) {
            _staged = new Move();
        }
     ...
     * one square.
     * Assume either move0-mov1 or move1-move0 is a valid jump.
     */
    static Move move(Move move0, Move move1) {
        ...
        if (move0.jumpTail() == null) {
            if (move1.jumpTail() != null) {
                return Move.parseMove(move0.toString()
                        + '-' + move1.jumpTail().toString());
            } else {
                return Move.parseMove(move0.toString()
     ...
     */
    boolean isLeftMove() {
        return (!isJump() && Character.compare(col1(), col0()) < 0
                    && Character.compare(row1(), row0()) == 0);
    }
    ...
    char jumpedRow() {
        if (isJump()) {
            int row0 = index(col0(), row0());
            int row1 = index(col1(), row1());
            int index = Math.abs(row1 - row0);
            return row(index);
        }
        return row1();
    }
    ...
    char jumpedCol() {
        if (isJump()) {
            int col0 = index(col0(), row0());
            int col1 = index(col1(), row1());
            int index = Math.abs(col1 - col0);
            return col(index);
        }
        return col1();
    }
    ...
    public String toString() {
        ...
        S.append(row0());
        S.append('-');
        if (isJump() && jumpTail() != null) {
            S.append(jumpTail().toString());
        } else {
            S.append(col1());
            S.append(row1());
        }
